...
const teklifDetay = `
select 
(case Erp_QuotationReceiptItem.IsClosed when 0 then 'Sipariş Bekleniyor' when 1 then 'Siparişe Dönmüş' when null then 'Sipariş Bekleniyor' ELSE ''END) as [Açık / Kapalı],
ReceiptNo as [Teklif No],
ProjectCode as [Proje No],
Ud_OrtakTeklif as [Ortak Teklif No],
Erp_QuotationReceiptItem.ReceiptDate as [Fiş Tarihi],
Erp_QuotationReceiptItem.DeliveryDate as [Müşteri İsteme Tarihi],
substring(Erp_CurrentAccount.CurrentAccountName,1,10) As Müşteri,
Erp_QuotationReceipt.Ud_Ihracatyurtici as [Ihracat/Yurt İçi],
...
InventoryCode as [Malzeme Kodu],
InventoryName as [Malzeme Adı],
Erp_QuotationReceiptItem.NER_Assortment as [Katalog Kodu],
SizeDetailCode as [Ölçü Kodu],
Erp_QuotationReceiptItem.NER_RENKDETAY as [Renk],

Cast(Erp_QuotationReceiptItem.NetQuantity as DECIMAL(18,2)) [Adet],
Erp_QuotationReceiptItem.NER_Unit2 as [2.Birim],
Cast(Erp_QuotationReceiptItem.Quantity as DECIMAL(18,2)) [Miktar],
Meta_UnitSetItem.UnitCode as [Birim],

Cast(Erp_QuotationReceiptItem.UnitPrice as DECIMAL(18,3)) Fiyat,
Cast(Erp_QuotationReceiptItem.ItemTotal as DECIMAL(18,2)) Tutar,

Erp_QuotationReceipt.UD_FiyatOnay as [Fiyat Onay],
(case Erp_QuotationReceipt.IsApproved when 0 then 'Onay Bekliyor' when 1 then 'Onaylandı' ELSE ''END) as [Teklif Onay]
...
Inner Join Erp_InventoryUnitItemSizeSetDetails On Erp_InventoryUnitItemSizeSetDetails.RecId = Erp_QuotationReceiptItem.InventoryUnitItemSizeSetDetailsId
Inner Join Erp_CurrentAccount On Erp_CurrentAccount.RecId = Erp_QuotationReceipt.CurrentAccountId
Left Join Meta_UnitSetItem On Meta_UnitSetItem.RecId = Erp_QuotationReceiptItem.UnitId
Left Join Erp_Project On Erp_Project.RecId = Erp_QuotationReceipt.ProjectId
Left Join Erp_InitialCost On Erp_InitialCost.RecId = Erp_QuotationReceiptItem.InitialCostId
